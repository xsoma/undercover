...
void LagComp::LagRecord_t::BuildBones( Player* target, bool resolve ) {
	...
	const auto backup_bone = target->GetBoneArrayForWrite();
	target->GetBoneArrayForWrite() = m_pMatrix_Resolved;

	target->SetupBones(nullptr, -1, 0x7FF00, m_flSimulationTime);

	target->GetBoneArrayForWrite() = backup_bone;
